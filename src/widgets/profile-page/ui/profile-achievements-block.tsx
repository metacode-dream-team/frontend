...
export function ProfileAchievementsBlock({ profileSlug, achievements }: ProfileAchievementsBlockProps) {
  ...
  const mobilePreview = previewAchievements(achievements, 3);
  const desktopPreview = previewAchievements(achievements, 4);

  return (
    <section className="flex w-full flex-col rounded-2xl bg-zinc-900/55 px-3 py-2 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.65)] backdrop-blur-[2px] max-lg:px-3 max-lg:py-2.5 sm:px-4 xl:py-2.5">
      ...
      <div className="mt-2.5 flex flex-wrap items-center justify-center gap-3 pl-0 max-lg:gap-2.5 xl:hidden xl:justify-start xl:pl-2">
        {mobilePreview.map((a) => (
          <AchievementLogo key={a.id} achievement={a} size="lg" className="max-lg:h-24 max-lg:w-24" />
        ))}
      </div>
